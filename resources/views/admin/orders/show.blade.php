...
@section('content')
    <div class="bg-white rounded-2xl shadow-md p-6 border border-[#e0e8d5]">
        ...
        <div class="flex justify-between items-start mb-6">
            ...
            @auth
                @if (Auth::user()->role_id === Role::ADMINISTRADOR || Auth::user()->role_id === Role::REPARTIDOR)
                    <form action="{{ route('admin.orders.update-status', $order) }}" method="POST"
                        ...
                        <button type="submit" class="bg-[#5c8b2d] hover:bg-[#3a5e1e] text-white px-3 py-1 rounded-lg text-sm">
                            Actualizar
                        </button>
                    </form>
                @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-[#e0e8d5]">
                <thead class="bg-[#f6fbee]">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#5c8b2d] uppercase tracking-wider">Producto
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#5c8b2d] uppercase tracking-wider">Precio
                            Unitario</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#5c8b2d] uppercase tracking-wider">Cantidad
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#5c8b2d] uppercase tracking-wider">Subtotal
                        </th>
                    </tr>
                </thead>
